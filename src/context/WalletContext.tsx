...
export const WalletProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Auto-prompt to connect - disabled (only show once on load)
  useEffect(() => {
    // Disabled: Only show prompt once on initial load
    return () => {};
  }, [walletState.isConnected]);

  // Auto-connect on mount if MetaMask is available and was previously connected
  useEffect(() => {
    const autoConnect = async () => {
      if (typeof window.ethereum !== 'undefined') {
        try {
          ...
          // If user was previously connected, auto-connect
          if (accounts.length > 0) {
            ...
            // Show connect prompt immediately if not connected
            setTimeout(() => {
          ...
          // Show connect prompt on error
          setTimeout(() => {
            setShowConnectPrompt(true);
          }, 1500);
        }
      } else {
        // MetaMask not installed - show prompt
        setTimeout(() => {
          setShowConnectPrompt(true);
        }, 1000);
      }
